...
class UniChem(BaseModel):
    chebi: str | SkipJsonSchema[None] = Field(default=None, title="ChEBI", description="ChEBI is a freely available ontology of molecular entities focused on 'small' chemical compounds")  
    chembl: str | SkipJsonSchema[None] = Field(default=None, title="ChEMBL", description="A database of bioactive drug-like small molecules and bioactivities abstracted from the scientific literature.") 
    lincs: str | SkipJsonSchema[None] = Field(default=None, title="LINCS", description="The LINCS DCIC facilitates and standardizes the information relevant to LINCS assays as described in http://www.lincsproject.org/data/data-standards/")
    pubchem: str | SkipJsonSchema[None] = Field(default=None, title="PubChem", description="A database of normalized PubChem compounds (CIDs) from the PubChem Database.")
    drugbank: str | SkipJsonSchema[None] = Field(default=None, title="DrugBank", description="A database that combines drug (i.e. chemical, pharmacological and pharmaceutical) data with drug target (i.e. sequence, structure, and pathway) information.")
...
class ChemblDrugMechanism(BaseModel):
    actionType: str | SkipJsonSchema[None] = Field(default=None, title="Action Type")
    targetComponents: list[ChemblTargetComponent] | SkipJsonSchema[None] = Field(default=None, title="Target Components")
    targetName: str | SkipJsonSchema[None] = Field(default=None, title="Target Name")
...
class PubChem(BaseModel):
    cid: str | SkipJsonSchema[None] = Field(default=None, title="CID")
    inchi: str | SkipJsonSchema[None] = Field(default=None, title="InChI")
    # inchikey: str = Field(default=None, title="InChI Key")  
...
class SmallMolecule(BaseModel):
    ...
    pubchem: PubChem | SkipJsonSchema[None] = Field(default=None, title="PubChem", json_schema_extra={"type": "object"})
